...
const ProfileEditForm = () => {
  ...
  const { screen_name, full_name, profile_image } = profileData;
    ...
    const handleMount = async () => {
      if (currentUser?.profile_id?.toString() === id) {
        try {
          ...
          const { screen_name, full_name, profile_image } = data;
          setProfileData({ screen_name, full_name, profile_image });
        } catch (err) {
          console.log(err);
          history.push("/");
        }
  ...
  const textFields = (
    <>
      ...
      <Button className={`${btnStyles.Button} ${btnStyles.Blue}`} type="submit">save</Button>
    </>
  );
